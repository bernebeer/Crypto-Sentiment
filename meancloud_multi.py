import requests, json, tweepy, re, time
from config import *
...
def main(numpages, numtweets):
	...
	for item in cglist:
		for symbol in symbols:
			if symbol[1:].lower() == item['symbol'] and 'peg' not in item['id']:
				...
				cgid = item['id']
				results[cgid] = {'symbol':symbol, 'cgid':cgid, 'avgsentiment':0, 'terms':{}, '24hr':0, 'mentions':0, 'tweets':{}}
	...
	for uniquetweet in uniquetweets:
		...
		# check if term value in text, do stuff
		for key, value in results.items():
	...
	for key, value in results.items():
		print('%-6s price: $%-8.2f 24hr: %-7.2f sent: %-4s mentions: %-4d' % (results[key]['symbol'],results[key]['price'],results[key]['24hr'],results[key]['avgsentiment'], len(results[key]['tweets'])))
